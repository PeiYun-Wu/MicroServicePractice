...
namespace Mango.Services.ShoppingCartAPI.Repository
{
    public class CartRepository : ICartRepository
    {
        ...
        {
            var cartFromDb = await _db.CartHeaders.FirstOrDefaultAsync(u => u.UserId == userId);  //userid=null  解決
        ...
        public async Task<CartDto> CreateUpdateCart(CartDto cartDto)
        {
            ...
            //check if product exists in DB , if not creat it
            ...
            if (prodInDb == null)
            {
                ...
                await _db.SaveChangesAsync();  //儲存DB  
            }

            //check header if is null
            var cartHeaderFromDb = await _db.CartHeaders.AsNoTracking().FirstOrDefaultAsync(u => u.UserId == cart.CartHeader.UserId); //用AsNoTracking, 因為沒更新現有db內容
            if (cartHeaderFromDb == null)
            {
        ...
        public async Task<bool> RemoveFromCart(int cartDetailsId)
        {
            try
            {
                ...
                if (totalCountOfCartItems == 1) //cart裡最後一個產品
                {
